    ...
    <?php if(isset($members) && isset($sno)): $i=$sno + 1;?>
                ...
                <?php foreach ($members as $key => $value): ?>
                  ...
                  <tr >
                    ...
                    <button <?php if($value->MemberRole != 'Head'){echo "hidden";}?> onclick="showupdatemembermodal('<?=trim($value->Familymembershipid)?>')" style='width:30px;height:30px;outline:none;border:none;' class='updatecoord shadow-sm text-dark table-btn rounded-circle'><i class='fa-regular fa-pen-to-square'></i><span class='updatetooltip'>Update Details</span></button>
                    <button data-bs-toggle='modal' data-bs-target='#deletemodal' onclick="showRejectMemberModal('<?=trim($value->Id)?>','<?=trim($value->Name)?>','<?=trim($value->Taluk)?>')" style='width:30px;height:30px;outline:none;border:none;color:red;' class='trashcoord table-btn shadow-sm rounded-circle'><i class='fa-solid fa-user-xmark'></i><span class='trashtooltip'>Reject</span></button>
