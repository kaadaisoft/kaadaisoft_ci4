...
<?php if(isset($coordinators) && isset($sno)):?>
    ...
    <?php $j = $sno+1; foreach ($coordinators as $key => $coordinator):?> 
                 ...
                 <tr>
                    ...
                    <button onclick="showupdatecoordsmodal('<?=trim($coordinator->Familymembershipid)?>')" style='width:30px;height:30px;outline:none;border:none;' class='updatecoord shadow-sm text-dark table-btn rounded-circle'><i class='fa-regular fa-pen-to-square'></i><span class='updatetooltip'>Update</span></button>
                    ...
                    <button onclick ="viewCoordinatordata('viewCoordinatordata?coord_id=<?=$coordinator->Familymembershipid?>')" data-bs-toggle='tooltip' title='viewCoordinatordata' style='width:30px;height:30px;outline:none;border:none;' class='table-btn text-dark shadow-sm rounded-circle'><i class='fa-sharp fa-solid fa-eye'></i></button>
                    </td>
                    </tr>
                  <?php ++$j; endforeach;?>
                <?php else:?>
